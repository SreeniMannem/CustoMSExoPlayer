...
/* package */ final class RtspClient implements Closeable {
  ...
  public RtspClient(
      ...
      String userAgent,
      Uri uri) {
    Log.e(TAG, "rtsp RtspClient userAgent: "+userAgent+", Uri: "+uri.toString());
  ...
  public void start() throws IOException {
    try {
      ...
      Log.e(TAG, "rtsp messageChannel.open(getSocket(uri)) exception: "+e.getMessage());
  ...
  private static Socket getSocket(Uri uri) throws IOException {
    ...
    Log.e(TAG, "rtsp Port: "+rtspPort+", uri.getHost(): "+uri.getHost());
  ...
  private static ImmutableList<RtspMediaTrack> buildTrackList(
    ...
    for (int i = 0; i < sessionDescription.mediaDescriptionList.size(); i++) {
      ...
      Log.e(TAG, "rtsp buildTrackList mediaDescription: "+mediaDescription.mediaType+", payloadType: "
          +mediaDescription.payloadType+", "+mediaDescription.toString()+", "+mediaDescription.rtpMapAttribute.mediaEncoding);
      // Includes tracks with supported formats only.
      if (RtpPayloadFormat.isFormatSupported(mediaDescription)) {
        Log.e(TAG, "rtsp buildTrackList isFormatSupported and added : "+uri);
  ...
  private final class MessageListener implements RtspMessageChannel.MessageListener {
    ...
    private void handleRtspMessage(List<String> message) {
      ...
      Log.e(TAG, "rtsp handleRtspMessage response headers: "+response.headers.toString());
      Log.e(TAG, "rtsp handleRtspMessage response: "+response.messageBody+", status: "+response.status);
      ...
      try {
        ...
        Log.e(TAG, "rtsp handleRtspMessage requestMethod: "+requestMethod);
    ...
    private void onDescribeResponseReceived(RtspDescribeResponse response) {
      Log.e(TAG, "rtsp onDescribeResponseReceived response: "+response.status+" ,"+response.sessionDescription);
      ...
      Log.e(TAG, "rtsp onDescribeResponseReceived sessionRangeAttributeString: "+sessionRangeAttributeString);
      ...
      Log.e(TAG, "rtsp onDescribeResponseReceived response.sessionDescription: "+response.sessionDescription+", uri+ "+uri);
      ImmutableList<RtspMediaTrack> tracks = buildTrackList(response.sessionDescription, uri);
      Log.e(TAG, "rtsp onDescribeResponseReceived tracks.size(): "+tracks.size());
      if (tracks.isEmpty()) {
    ...
    private void onPlayResponseReceived(RtspPlayResponse response) {
      Log.e(TAG, "rtsp onPlayResponseReceived response: "+response.trackTimingList.size());
      if (keepAliveMonitor == null) {
