...
public class DefaultRenderersFactory implements RenderersFactory {
  ...
  public DefaultRenderersFactory setExtensionRendererMode(
      @ExtensionRendererMode int extensionRendererMode) {
    Log.e(TAG, "rtsp setExtensionRendererMode extensionRendererMode: "+extensionRendererMode);
    this.extensionRendererMode = extensionRendererMode;
    return this;
  }
  ...
  protected void buildAudioRenderers(
      ...
      ArrayList<Renderer> out) {
    Log.i(TAG, "rtsp buildAudioRenderers out size: "+out.size());
    /*FfmpegAudioRenderer ffmpegAudioRenderer =
        new FfmpegAudioRenderer(eventHandler, eventListener, audioSink);
    ffmpegAudioRenderer.supportsMixedMimeTypeAdaptation();
    ffmpegAudioRenderer.experimentalSetEnableKeepAudioTrackOnSeek(true);*/
    ...
    Log.i(TAG, "rtsp buildAudioRenderers extensionRendererMode: "+extensionRendererMode);
    if (extensionRendererMode == EXTENSION_RENDERER_MODE_OFF) {
      return;
    }
    int extensionRendererIndex = out.size();
    Log.i(TAG, "rtsp buildAudioRenderers extensionRendererIndex: "+extensionRendererIndex);
    if (extensionRendererMode == EXTENSION_RENDERER_MODE_PREFER) {
      extensionRendererIndex--;
    }
    ...
    try {
      ...
      Renderer renderer =
          (Renderer) constructor.newInstance(eventHandler, eventListener, audioSink);
      out.add(extensionRendererIndex++, renderer);
      Log.i(TAG, "rtsp Loaded FfmpegAudioRenderer.");
    } catch (ClassNotFoundException e) {
      // Expected if the app was built without the extension.
    } catch (Exception e) {
      // The extension is present, but instantiation failed.
